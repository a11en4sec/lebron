...
var (
	...
	cacheUserUserPhonePrefix    = "cache:user:user:phone:"
	cacheUserUserUsernamePrefix = "cache:user:user:username:"
)
...
func (m *defaultUserModel) Delete(ctx context.Context, id int64) error {
	...
	userUserPhoneKey := fmt.Sprintf("%s%v", cacheUserUserPhonePrefix, data.Phone)
	userUserUsernameKey := fmt.Sprintf("%s%v", cacheUserUserUsernamePrefix, data.Username)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, userUserIdKey, userUserPhoneKey, userUserUsernameKey)
	return err
}
...
func (m *defaultUserModel) FindOneByPhone(ctx context.Context, phone string) (*User, error) {
	userUserPhoneKey := fmt.Sprintf("%s%v", cacheUserUserPhonePrefix, phone)
	var resp User
...
func (m *defaultUserModel) FindOneByUsername(ctx context.Context, username string) (*User, error) {
	...
	var resp User
	err := m.QueryRowIndexCtx(ctx, &resp, userUserUsernameKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) (i interface{}, e error) {
