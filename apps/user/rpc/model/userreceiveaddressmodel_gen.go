...
var (
	...
	cacheUserUserReceiveAddressIdPrefix = "cache:user:userReceiveAddress:id:"
)
...
func (m *defaultUserReceiveAddressModel) Delete(ctx context.Context, id int64) error {
	userUserReceiveAddressIdKey := fmt.Sprintf("%s%v", cacheUserUserReceiveAddressIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, userUserReceiveAddressIdKey)
	return err
}
...
func (m *defaultUserReceiveAddressModel) FindOne(ctx context.Context, id int64) (*UserReceiveAddress, error) {
	...
	err := m.QueryRowCtx(ctx, &resp, userUserReceiveAddressIdKey, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) error {
...
func (m *defaultUserReceiveAddressModel) Insert(ctx context.Context, data *UserReceiveAddress) (sql.Result, error) {
	userUserReceiveAddressIdKey := fmt.Sprintf("%s%v", cacheUserUserReceiveAddressIdPrefix, data.Id)
...
func (m *defaultUserReceiveAddressModel) formatPrimary(primary interface{}) string {
	return fmt.Sprintf("%s%v", cacheUserUserReceiveAddressIdPrefix, primary)
}
